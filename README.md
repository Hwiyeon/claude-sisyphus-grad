...
- **2026-04-21** — `/train` loop overhead trimmed: batched GitHub sync (every 5 epochs + event-based triggers instead of per-epoch), enforced parallel reviewer calls in a single message for Round 2/3, and `POLL_INTERVAL` between iterations reduced from 10s to 2s.
