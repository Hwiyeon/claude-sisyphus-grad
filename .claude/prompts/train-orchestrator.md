...
- **No background agents for monitoring**: do not use `run_in_background` Tasks or Agent calls for epoch polling. Background **Bash** (a shell script returning "EPOCH N" / "CRASH" / "TIMEOUT") is the polling mechanism — zero API calls inside the shell. However, **foreground subagents for per-epoch processing are permitted** (and required): once epochs are detected, delegate metric parsing, state recording, detail.md writing, and git sync to a foreground Agent call. See `train-monitor.md` Per-Epoch Processing Procedure for details.
